...
export class AlarmRuleEngineService {
   ...
   * @returns 告警评估结果
   */
  async evaluateData(data: ParsedDataDocument): Promise<AlarmEvaluationResult> {
   ...
   * 评估单条规则
   */
  private async evaluateRule(rule: AlarmRuleDocument, data: ParsedDataDocument): Promise<AlarmDocument | null> {
    switch (rule.type) {
      case 'threshold':
   ...
   */
  private evaluateThresholdRule(rule: AlarmRuleDocument, data: ParsedDataDocument): AlarmDocument | null {
   ...
   */
  private evaluateConstantRule(rule: AlarmRuleDocument, data: ParsedDataDocument): AlarmDocument | null {
   ...
   * TODO: 实现安全的脚本执行环境
   */
  private evaluateCustomRule(_rule: AlarmRuleDocument, _data: ParsedDataDocument): AlarmDocument | null {
