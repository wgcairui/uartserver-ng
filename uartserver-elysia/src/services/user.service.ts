...
export class UserService {
  ...
  async hasDeviceAccess(userId: string, deviceMac: string): Promise<boolean> {
    console.log('[UserService.hasDeviceAccess] Checking userId:', userId, 'deviceMac:', deviceMac);
    const user = await this.getUserById(userId);
    console.log('[UserService.hasDeviceAccess] User found:', user ? `${user.username} (devices: ${JSON.stringify(user.devices)})` : 'null');
    if (!user) return false;
    ...
    const hasAccess = user.devices?.includes(deviceMac) || false;
    console.log('[UserService.hasDeviceAccess] hasAccess:', hasAccess);
    return hasAccess;
  }
}
...
/**
 * 获取 UserService 单例实例
 * 使用延迟初始化避免在模块加载时访问数据库
 */
export function getUserService(): UserService {
  if (!userServiceInstance) {
    userServiceInstance = new UserService(mongodb.getDatabase());
  }
